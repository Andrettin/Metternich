		...
		function get_commodity_outputs_string(commodity_outputs) {
			...
			for (var kv_pair of commodity_outputs) {
				...
				if (commodity === metternich.defines.wealth_commodity) {
					...
					str += commodity.name + " Output"
				}
				
				str += ": " + commodity.value_to_qstring(output)
			}
		...
		IconButton {
			...
			onHoveredChanged: {
				if (hovered) {
					...
					middle_status_text = pathway.get_commodity_costs_string_for_province(selected_civilian_unit.province, true)
