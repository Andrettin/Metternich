...
Item {
	...
	MouseArea {
		...
		hoverEnabled: true
		enabled: garrison_icon.visible && province !== null && ((province.water_zone && province.game_data.military_unit_category_counts.length > 0 && province.game_data.get_domain_military_unit_category_counts(metternich.game.player_country).length > 0) || (province.game_data.owner !== null && (province.game_data.owner === metternich.game.player_country || province.game_data.owner.game_data.is_any_vassal_of(metternich.game.player_country))))
		visible: enabled
