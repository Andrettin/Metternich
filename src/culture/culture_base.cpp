...
void culture_base::process_gsml_scope(const gsml_data &scope)
{
	...
	if (tag == "patronyms") {
		...
		scope.for_each_child([this](const gsml_data &child_scope) {
			...
			const gender gender = magic_enum::enum_cast<archimedes::gender>(tag).value();
...
{
	auto government_find_iterator = this->title_names.find(government_type);
	if (government_find_iterator == this->title_names.end()) {
		government_find_iterator = this->title_names.find(government_type->get_group());
	}

	if (government_find_iterator != this->title_names.end()) {
		const std::string &title_name = government_type::get_title_name(government_find_iterator->second, tier);
		if (!title_name.empty()) {
			return title_name;
		}
	}
