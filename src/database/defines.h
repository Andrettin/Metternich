...
class defines final : public defines_base, public singleton<defines>
{
	...
	Q_PROPERTY(int province_map_tile_scale MEMBER province_map_tile_scale READ get_province_map_tile_scale NOTIFY changed)
...
public:
	...
	decimillesimal_int days_to_turns(const int days, const int current_year) const
	{
		return decimillesimal_int(days) / 30 / this->get_months_per_turn(current_year);
	}

	decimillesimal_int months_to_turns(const decimillesimal_int &months, const int current_year) const
	{
		return months / this->get_months_per_turn(current_year);
	}

	decimillesimal_int months_to_turns(const int months, const int current_year) const
	{
		return this->months_to_turns(decimillesimal_int(months), current_year);
	}

	decimillesimal_int years_to_turns(const int years, const int current_year) const
	{
	...
	int get_battle_map_scale() const
	{
	...
	const commodity *get_regency_commodity() const
	{
		return this->regency_commodity;
	}
	...
	int64_t get_research_cost_per_level() const
	{
	...
	const domain_skill *get_main_research_domain_skill() const
	{
	...
	const office *get_heir_office() const
	{
	...
	int get_province_map_tile_scale() const
	{
		return this->province_map_tile_scale;
	}
...
private:
	...
	const commodity *regency_commodity = nullptr;
