...
QCoro::Task<void> game::apply_sites()
{
	...
	for (const site *site : map::get()->get_sites()) {
		...
		if (site_history->get_owner() == nullptr && site_game_data->get_holding_type() != nullptr) {
			if (site_game_data->get_holding_type()->is_economic() && site_province->get_history()->get_trade_zone() != nullptr) {
...
QCoro::Task<void> game::on_setup_finished()
{
	...
	for (const domain *domain : this->get_domains()) {
		...
		for (const province *province : domain_game_data->get_provinces()) {
			for (const site *site : province->get_game_data()->get_settlement_sites()) {
