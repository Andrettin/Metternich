...
void map::initialize()
{
	...
	for (const province *province : this->get_provinces()) {
		resource_map<int> resource_counts = province->get_resource_counts();

		//remove the already-existing resource tiles from the counts
		for (const auto &[resource, count] : province->get_map_data()->get_resource_counts()) {
		...
		for (const auto &[resource, count] : resource_counts) {
			...
			for (const site *site : province->get_map_data()->get_sites()) {
				...
				const QPoint &tile_pos = site->get_map_data()->get_tile_pos();

				if (resource->is_coastal() && !this->is_tile_coastal(tile_pos)) {
					continue;
				}

				if (resource->is_near_water() && !this->is_tile_near_water(tile_pos)) {
					continue;
				}
			...
			for (int i = 0; i < count; ++i) {
				...
				const QPoint &tile_pos = resource_site->get_map_data()->get_tile_pos();
				tile *tile = this->get_tile(tile_pos);

				province_map_data *province_map_data = province->get_map_data();

				if (!vector::contains(resource->get_terrain_types(), tile->get_terrain())) {
					const terrain_type *fallback_terrain = resource->get_fallback_terrain(tile->get_terrain());

					if (fallback_terrain != nullptr) {
						if (tile->get_terrain() != nullptr) {
							province_map_data->change_tile_terrain_count(tile->get_terrain(), -1);
						}

						this->set_tile_terrain(tile_pos, fallback_terrain);
				...
				resource_site->get_map_data()->set_type(site_type::resource);
				resource_site->get_map_data()->set_resource(resource);
				province_map_data->on_resource_tile_added(tile_pos);
			}
		}
	}
