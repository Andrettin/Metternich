...
{
	connect(map::get(), &map::tile_terrain_changed, this, &combat_map_grid_model::on_tile_terrain_changed);
	connect(map::get(), &map::tile_holding_type_changed, this, &combat_map_grid_model::on_tile_holding_type_changed);
	connect(map::get(), &map::tile_improvement_changed, this, &combat_map_grid_model::on_tile_improvement_changed);
	connect(map::get(), &map::tile_civilian_unit_changed, this, &combat_map_grid_model::on_tile_civilian_unit_changed);
}
		...
		switch (model_role) {
			case role::base_image_sources: {
				...
				if (!defines::get()->get_default_base_terrain()->get_subtiles().empty()) {
					...
					image_sources.push_back(combat_map_grid_model::build_image_source(defines::get()->get_default_base_terrain(), tile->get_base_tile()));
				}
			...
			case role::image_sources: {
				...
				if (!tile->get_terrain()->get_subtiles().empty()) {
					for (const short subtile : tile->get_subtiles()) {
						image_sources.push_back(combat_map_grid_model::build_image_source(tile->get_terrain(), subtile));
					}
				} else {
					image_sources.push_back(combat_map_grid_model::build_image_source(tile->get_terrain(), tile->get_tile()));
				}
			...
			case role::object_image_sources: {
				...
				if (!game::get()->get_player_country()->get_game_data()->is_tile_explored(tile_pos)) {
					object_image_sources.push_back(combat_map_grid_model::build_image_source(defines::get()->get_unexplored_terrain(), 0));
				}
...
void combat_map_grid_model::on_tile_terrain_changed(const QPoint &tile_pos)
{
...
void combat_map_grid_model::on_tile_holding_type_changed(const QPoint &tile_pos)
{
	const QModelIndex index = this->index(tile_pos.y(), tile_pos.x());
	emit dataChanged(index, index, {
...
void combat_map_grid_model::on_tile_improvement_changed(const QPoint &tile_pos)
{
	const QModelIndex index = this->index(tile_pos.y(), tile_pos.x());
	emit dataChanged(index, index, {
		static_cast<int>(role::object_image_sources),
...
void combat_map_grid_model::on_tile_civilian_unit_changed(const QPoint &tile_pos)
{
	const QModelIndex index = this->index(tile_pos.y(), tile_pos.x());
	emit dataChanged(index, index, {
