...
QCoro::Task<void> province_game_data::set_owner(const domain *domain)
{
	...
	if (this->get_owner() != nullptr) {
		...
		this->population->add_upper_population(this->get_owner()->get_game_data()->get_country_population());
...
const site *province_game_data::get_best_provincial_capital_slot() const
{
	...
	for (const site *site : this->province->get_map_data()->get_settlement_sites()) {
		...
		if (!government_type->is_holding_type_allowed(site->get_holding_type())) {
			continue;
		}
		...
		if (this->get_owner() != nullptr && site == this->get_owner()->get_default_capital()) {
			...
			if (found_preferred_holding_type) {
				if (!government_type->is_holding_type_preferred(site->get_holding_type())) {
					continue;
				}
			} else if (government_type->is_holding_type_preferred(site->get_holding_type())) {
				potential_provincial_capitals.clear();
				found_preferred_holding_type = true;
				best_holding_level = site->get_game_data()->get_holding_level();
			}
...
{
	const centesimal_int divisor = centesimal_int::max(centesimal_int::max(this->get_total_holding_level(), this->get_settlement_count()), 1);
	assert_throw(divisor > 0);

	return (defines::get()->get_province_population_for_level(this->get_level()) * centesimal_int::max(holding_level, 1) / divisor).to_int64();
}
