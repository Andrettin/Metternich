...
void factor_modifier<scope_type>::process_gsml_property(const gsml_property &property)
{
	...
	} else if (std::is_same_v<scope_type, province> && key == "province_level_scale") {
		if (gsml_operator == gsml_operator::assignment) {
			this->province_level_scale = std::stoi(value);
		} else {
			throw std::runtime_error(std::format("Invalid operator for property (\"{}\").", property.get_key()));
		}
	} else {
...
{
	std::unique_ptr<const condition_base<scope_type, read_only_context>> condition = metternich::condition<scope_type>::from_gsml_scope(scope);
	this->conditions->add_condition(std::move(condition));
}

template <typename scope_type>
void factor_modifier<scope_type>::process_gsml_data(const gsml_data &data)
{
	data.process(this);
}
...
template class factor_modifier<domain>;
template class factor_modifier<military_unit>;
