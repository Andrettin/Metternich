...
std::unique_ptr<modifier_effect<scope_type>> modifier_effect<scope_type>::from_gsml_property(const gsml_property &property)
{
	...
	if constexpr (std::is_same_v<scope_type, const character>) {
		...
		if (key == "artillery_cost_modifier") {
			...
			return std::make_unique<max_current_constructions_modifier_effect>(value);
			...
			return std::make_unique<domain_attribute_modifier_effect>(domain_attribute::get(key), value);
		...
		if (key == "base_population_capacity") {
			...
			return std::make_unique<population_capacity_modifier_modifier_effect>(value);
...
std::unique_ptr<modifier_effect<scope_type>> modifier_effect<scope_type>::from_gsml_scope(const gsml_data &scope)
{
	...
	scope.process(modifier_effect.get());
