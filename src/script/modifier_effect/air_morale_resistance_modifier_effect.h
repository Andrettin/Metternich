...
public:
	...
	{
		scope->get_game_data()->change_air_morale_resistance_modifier(this->quantity * multiplier);
	}

	virtual std::string get_string(const int multiplier) const override
	{
		return "Air Morale Resistance: " + number::to_signed_string(this->quantity * multiplier) + "%";
	}
