...
public:
	...
	{
		scope->get_game_data()->change_naval_morale_resistance_modifier((this->quantity * multiplier).to_int());
	}

	virtual std::string get_string(const centesimal_int &multiplier) const override
	{
		return std::format("Naval Morale Resistance: {}%", number::to_signed_string((this->quantity * multiplier).to_int()));
	}
