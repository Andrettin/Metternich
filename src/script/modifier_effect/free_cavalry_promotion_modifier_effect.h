...
class free_cavalry_promotion_modifier_effect final : public modifier_effect<const domain>
{
public:
	explicit free_cavalry_promotion_modifier_effect(const std::string &value)
	{
	...
	{
		static const std::string identifier = "free_cavalry_promotion";
		return identifier;
	}

	[[nodiscard]] virtual QCoro::Task<void> apply_coro(const domain *scope, const centesimal_int &multiplier) const override
	{
		co_await scope->get_military()->change_free_cavalry_promotion_count(this->promotion, (this->value * multiplier).to_int());
	}
	...
	virtual std::string get_base_string(const domain *scope) const override
	{
		...
		return std::format("Free Cavalry Promotion: {}", this->promotion->get_name());
	}
