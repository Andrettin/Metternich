...
class piety_effect final : public effect<scope_type>
{
public:
	...
	virtual void do_addition_effect(const scope_type *scope) const override
	{
		...
		if constexpr (std::is_same_v<scope_type, const character>) {
			if (scope->get_game_data()->is_ruler()) {
				//piety changes for rulers should also affect their countries
				scope->get_game_data()->get_employer()->get_game_data()->change_piety(this->quantity);
			}
		}
	}
