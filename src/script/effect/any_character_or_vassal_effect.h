...
class any_character_or_vassal_effect final : public scope_effect_base<upper_scope_type, const character>
{
public:
	explicit any_character_or_vassal_effect(const gsml_operator effect_operator)
	...
	virtual void do_assignment_effect(const upper_scope_type *upper_scope, context &ctx) const override
	{
		...
		for (const metternich::country *vassal_country : country->get_game_data()->get_vassals()) {
			...
			if (!this->conditions.check(vassal_country->get_game_data()->get_ruler(), ctx)) {
				continue;
			}
