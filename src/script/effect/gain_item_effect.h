...
class gain_item_effect final : public effect<scope_type>
{
public:
	explicit gain_item_effect(const gsml_operator effect_operator)
		: effect<scope_type>(effect_operator)
	{
	}

	explicit gain_item_effect(const std::string &value, const gsml_operator effect_operator)
		: gain_item_effect(effect_operator)
	{
	...
	{
		static const std::string class_identifier = "gain_item";
	...
	virtual void do_assignment_effect(scope_type *scope, context &ctx) const override
	{
		...
		auto item = make_qunique<metternich::item>(this->type, this->material, this->enchantment);
		if (!this->name.empty()) {
	...
	{
		const std::string item_name = !this->name.empty() ? this->name : item::create_name(this->type, this->material, this->enchantment);
