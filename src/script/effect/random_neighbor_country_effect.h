...
class random_neighbor_country_effect final : public scope_effect_base<const domain, const domain>
{
public:
	explicit random_neighbor_country_effect(const gsml_operator effect_operator)
	...
	{
		static const std::string class_identifier = "random_neighbor_country";
	...
	[[nodiscard]] virtual QCoro::Task<void> do_assignment_effect_coro(const domain *upper_scope, context &ctx) const override
	{
		...
		for (const domain *neighbor_country : upper_scope->get_game_data()->get_neighbor_countries()) {
			...
			potential_countries.push_back(neighbor_country);
		}

		if (!potential_countries.empty()) {
			co_await this->do_scope_effect(vector::get_random(potential_countries), upper_scope, ctx);
		}
	}
