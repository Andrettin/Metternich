...
class any_advisor_condition final : public scope_condition_base<upper_scope_type, character>
{
public:
	explicit any_advisor_condition(const gsml_operator condition_operator)
	...
	{
		static const std::string class_identifier = "any_advisor";
	...
	virtual bool check_assignment(const upper_scope_type *upper_scope, const read_only_context &ctx) const override
	{
		...
		const office_map<const character *> &office_characters = country->get_game_data()->get_office_characters();
