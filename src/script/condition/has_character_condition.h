...
class has_character_condition final : public condition<domain>
{
public:
	explicit has_character_condition(const std::string &value, const gsml_operator condition_operator)
	...
	{
		static const std::string class_identifier = "has_character";
	...
	virtual bool check_assignment(const domain *scope, const read_only_context &ctx) const override
	{
		...
		const character *character = this->get_character(ctx);

		if (character == nullptr) {
			return false;
		}

		return character->get_game_data()->get_domain() == scope;
	}
	...
	virtual std::string get_assignment_string(const size_t indent) const override
	{
		...
		if (std::holds_alternative<const character *>(this->character_target)) {
			...
			return std::format("Has {} character", character_name);
			...
			return "Has " + string::capitalized(std::string(magic_enum::enum_name<special_target_type>(target_type))) + " scope character";
		} else {
			assert_throw(false);
			return std::string();
		}
