...
public:
	...
	const modifier<const site> *get_construction_level_modifier(const construction_type construction_type, const int construction_level) const
	{
		const auto find_iterator = this->construction_level_modifiers.find(construction_type);
		if (find_iterator != this->construction_level_modifiers.end()) {
...
private:
	...
	std::map<construction_type, std::map<int, std::unique_ptr<const modifier<const site>>>> construction_level_modifiers;
