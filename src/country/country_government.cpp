...
QVariantList country_government::get_advisors_qvariant_list() const
{
	return container::to_qvariant_list(this->get_advisors());
}

void country_government::check_advisors()
{
	...
	if (this->get_game_data()->is_under_anarchy() || !this->can_have_advisors()) {
	...
	if (this->get_next_advisor() != nullptr) {
		if (this->get_next_advisor()->get_game_data()->get_country() != nullptr) {
			...
			if (this->country->get_economy()->get_stored_commodity(defines::get()->get_advisor_commodity()) >= this->get_advisor_cost()) {
				this->country->get_economy()->change_stored_commodity(defines::get()->get_advisor_commodity(), -this->get_advisor_cost());
		...
		if (this->country->get_economy()->get_commodity_output(defines::get()->get_advisor_commodity()).to_int() > 0 || this->country->get_economy()->get_stored_commodity(defines::get()->get_advisor_commodity()) > 0) {
			this->choose_next_advisor();
		}
	}
}

void country_government::add_advisor(const character *advisor)
{
	try {
...
void country_government::remove_advisor(const character *advisor)
{
...
void country_government::clear_advisors()
{
	const std::vector<const character *> advisors = this->get_advisors();
	for (const character *advisor : advisors) {
		this->remove_advisor(advisor);
...
void country_government::choose_next_advisor()
{
...
bool country_government::can_have_advisors() const
{
...
bool country_government::can_recruit_advisor(const character *advisor) const
{
...
bool country_government::has_incompatible_advisor_to(const character *advisor) const
{
...
const character *country_government::get_replaced_advisor_for(const character *advisor) const
{
	const std::vector<const character_trait *> advisor_traits = advisor->get_game_data()->get_traits_of_type(character_trait_type::advisor);
...
bool country_government::can_have_advisors_or_appointable_offices() const
{
